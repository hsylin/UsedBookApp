...
const url = "mongodb+srv://tsai960818:tAnW3GpC8vT2VW5F@cluster0.lf1woaw.mongodb.net/";
...
app.post("/getbean", (req, res) => {
	var token = req.header('x-auth');
	adminverify(token).then((flag) => {
		if (flag == 1) {
			bean.find({ bookID: req.body.bookID }, '-_id',).then((docs) => {
				return res.status(200).send(docs);
			}).catch(err => {
				return res.status(500).send("No Ad Available");
			});
...
app.post("/newbean", (req, res) => {
	var token = req.header('x-auth');
	var data = new bean(req.body);
	adminverify(token).then((flag) => {
		if (flag == 1) {
			data.save().then(doc => {
				return res.status(200).send(doc);
...
app.post("/updatebean", (req, res) => {
	var token = req.header('x-auth');
	adminverify(token).then((flag) => {
		if (flag == 1) {
			bean.findOneAndUpdate({ bookID: req.body.bookID }, { $set: { data: req.body.data } }).then((docs) => {
				return res.status(200).send(docs);
			}).catch(err => {
				return res.status(500).send("No Ad Available");
			});
